export const messages = {
  ...
  exceed_number_of_attempts: (name: string) => `Could not verify the service instance creation. Check its status using the 'cf service ${name}' command.`,
  service_not_found: (instanceName: string) => `Could not find the '${instanceName}' service instance.`,
  service_creation_failed: (error: string) => `Service instance creation failed: ${error}`,
  not_allowed_filter: (param: string, query: string) => `The '${param}' parameter is not allowed in the '${query}' query.`
};
